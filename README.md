...
현재 MVP에는 여덟 권의 책이 있습니다. 각 책은 새로 만든 표지, 글자 없는 8컷
줄거리 그림, 따라 읽을 여덟 문장과 연령별 퀴즈를 제공합니다. `Lv.1`(4–5세)은
그림과 핵심 낱말, `Lv.2`(6–7세)는 회상·관계·순서, `Lv.3`(8–9세)는 근거·비교·추론을
중심으로 각각 6·8·8문항을 구성합니다. 난이도는 `내 캐릭터`에서 선택하며 레벨별
완독 기록을 따로 저장합니다.
...
`안녕 마음아` 세 권(걱정 아저씨, 난 오줌 안 쌌어, 놀이터 귀신)의 표지와 8컷 만화는
아직 임시 그림입니다. 진짜 그림을 넣는 절차와 프롬프트는
[`docs/new-book-art-prompts.md`](docs/new-book-art-prompts.md)에 있습니다.
